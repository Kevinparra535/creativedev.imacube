...
export interface AttentionTarget {
  object: Object3D;         // Referencia al objeto Three.js
  type: TargetType;
...
export function scanForTargets(
  ...
  for (const { object, type } of availableTargets) {
    ...
    const interest = computeInterest(type, personality, distance, visitCount, isNew);
...
export function recordVisit(
  ...
  const existingIndex = targetHistory.findIndex(t => t.object === target.object);
